...
public class CourierEFormController implements Initializable {
	...
	@FXML
	private ComboBox<Destination> comboProv;
	@FXML
	private Label Dest_Prov;
	@FXML
	...
	public void annuler(ActionEvent event) {
		Stage stage=(Stage)comboProv.getScene().getWindow();
		stage.close();
	}
	...
	public void valider(ActionEvent event) {
		...
		if(nomDes!=null && prenomDest!=null &&telDest!=null && nomExp!=null && PrenomExp!=null &&
			...
			Courrier courier=new Courrier(null,codeColis.getText(),2, sdate, null, description.getText(),0.0, expediteur, destinataire);
			courier.setIdDestination(comboProv.getSelectionModel().getSelectedItem().getId());
			courier.setFrais(0.0);
			if(indicateur==0) {
				Long idMax;
				idMax=courierDAO.idMax();
				String code=String.format("COE-%04d", idMax);
				...
				if(Outils.confirmer("Voulez-vous vraiment enregistrer ces modifications?")) {
					...
					int index=ColisController.listecourierEntant.indexOf(tmpcourier);
					ColisController.listecourierEntant.set(index, courier);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		if(indicateur==1) {
			...
			comboProv.getSelectionModel().select(destination);
			...
			//valeur.setText(null);
			comboProv.getSelectionModel().select(null);
			//frais.setText(null);
