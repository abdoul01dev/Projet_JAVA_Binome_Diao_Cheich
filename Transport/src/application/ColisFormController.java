...
public class ColisFormController implements Initializable {
	...
	public void valider(ActionEvent event) {
		...
		if(nomDes!=null && prenomDest!=null &&telDest!=null && nomExp!=null && PrenomExp!=null &&
			...
			Colis colis=new Colis(null,codeColis.getText(),1, sdate, null, description.getText(),val, expediteur, destinataire);
			colis.setIdDestination(comboDest.getSelectionModel().getSelectedItem().getId());
			colis.setFrais(Double.valueOf(frais.getText()));
			if(indicateur==0) {
				colis=colisDAO.create(colis);
				ColisController.listecolisSortant.add(colis);
				...
				if(Outils.confirmer("Voulez-vous vraiment enregistrer ces modifications?")) {
					...
					int index=ColisController.listecolisSortant.indexOf(tmpcolis);
					ColisController.listecolisSortant.set(index, colis);
				}
			...
			TelExp.setText(null);;dateRecep.setValue(null); ;comboDest.getSelectionModel().select(null);
